...
df <- read.csv('Comal.csv',skip=0,header=TRUE)
head(df, 10)
...
plot(merged_df1_grouped$date, merged_df1_grouped$Discharge_cfs, xlab = "Date", ylab = "Discharge_cfs",main = "Timeseries graph (comal)", type = "l", col = "black")
...
#write.csv(merged_df1_grouped, "imputeddaycomal.csv")
...
#plotting time series data of well depth without na
plot(merged_df1_grouped$date, merged_df1_grouped$Discharge_cfs10d, xlab = "Date", ylab = "Discharge_cfs10d",main = "Timeseries graph (comal)",type = "l", col = "black")

#plotting acf
acf(merged_df1_grouped$Discharge_cfs10d, main = "Discharge_cfs10d", lag.max = 500, na.action = na.omit)
#plotting pacf
pacf(merged_df1_grouped$Discharge_cfs10d, main = "Discharge_cfs10d", lag.max = 500, na.action = na.omit)
